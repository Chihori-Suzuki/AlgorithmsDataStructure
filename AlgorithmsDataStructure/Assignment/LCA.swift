...
//
//  Created by Chiori on 2021/03/22.
//  Modified by Chiori & Daiki
//
...
func lca() {
    ...
    for _ in 0..<n2 {
        ...
        first.append(1)
        lcaHelper(adj, &first, targets[0], currentNode: 1)

        second.append(1)
        lcaHelper(adj, &second, targets[1], currentNode: 1)
...
@discardableResult
func lcaHelper(_ adj: [[Int]], _ visited: inout [Int],_ target: Int, currentNode: Int) -> Bool {
    ...
    for node in adj[currentNode] {
        ...
        visited.append(node)
        let isFound = lcaHelper(adj, &visited, target, currentNode: node)
